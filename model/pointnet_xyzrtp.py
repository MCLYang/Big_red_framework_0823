...
def get_model(input_channel,is_synchoization='BN'):
    return(Pointnet_xyzrtp_sem_seg(num_channel=input_channel,is_synchoization = is_synchoization))
...
def xyz2rtp(xyz):
    ...
    eps = 1e-10
    rtp = np.zeros((xyz.shape[0],3))
    #r
    rtp[:,0] = np.linalg.norm(xyz[:,:3],axis=1)
    #theta
    rtp[:,1] = np.arctan(xyz[:,1]/(xyz[:,0]+eps))
    #phi is ralated to laserID
    #phi
    rtp[:,2] = np.arccos(xyz[:,2]/(rtp[:,0]+eps))
    return rtp
def rtp2xyz(rtp):
    xyz = np.zeros((rtp.shape[0],3))
    xyz[:,0] = rtp[:,0]*np.cos(rtp[:,1])*np.sin(rtp[:,2])
    xyz[:,1] = rtp[:,0]*np.sin(rtp[:,1])*np.sin(rtp[:,2])
    xyz[:,2] = rtp[:,0]*np.cos(rtp[:,2])
    return(xyz)
...
class RingEncoder(nn.Module):
    def __init__(self, global_feat=False, feature_transform=True, channel=4,num_ring = 16,is_synchoization='BN'):
