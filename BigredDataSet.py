...
class BigredDataSet():
    def __init__(self,
            ...
            with h5py.File(os.path.join(root, file), 'r') as f:
                try:
                    ...
                    if (self.is_train == True and self.is_validation == False and self.is_test == False):
                        ...
                        self.file_dict[counter_for_file] = file
                        counter_for_file = counter_for_file + n_frame
                        end_dict[file] = counter_for_file
        ...
        import json
        json = json.dumps(end_dict)
        f = open("visulization_data/end_dict.json","w")
        f.write(json)
        f.close()

    def getVis(self,prediction):
        point_set = self.point_set
        label_set = self.label_set
        ermgering_set = np.copy(label_set)

        comparison = (label_set == prediction)
        comparison = comparison*1
        ermgering_set[comparison == 0] = 3
        # pdb.set_trace()
        return point_set,label_set,ermgering_set
